...
const ActivityPage = () => {
  ...
  const fetchAllEvents = async () => {
    try {
      ...
      if (res.data.success) {
        setEvents(res.data.data); // ✅ set all events
      }
    } catch (error) {
  ...
  const getStudentEventScore = (studentId, eventId) => {
    return scores.find(
      (s) => s.studentId === studentId && s.eventId === eventId,
    );
  };
  ...
  const toggleRegistration = (studentId, eventId, isRegistered) => {
    ...
    toast.success(
      `Student ${!isRegistered ? "registered" : "unregistered"} successfully`,
    );
  };
...
const handleCreateEvent = async (eventData) => {
  ...
  const EventStatusBadge = ({ status }) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${styles[status] || styles.upcoming}`}
      >
  ...
  const AdminEventsView = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {events.map((event) => (
        ...
        >
          <div className={`bg-gradient-to-r ${event.gradientColor} p-4`}>
            <h3 className="text-xl font-bold text-white mb-2">{event.title}</h3>
            <EventStatusBadge status={getEventStatus(event.date)} />
          </div>
          <div className="p-4">
            ...
            <div className="mt-4 flex justify-between items-center">
              <span className="text-xs text-gray-400">
                Scores: {scores.filter((s) => s.eventId === event._id).length}
              </span>
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <h2 className="text-2xl font-bold text-gray-900 mt-2">
            {selectedEvent?.title} - Scoreboard
          </h2>
